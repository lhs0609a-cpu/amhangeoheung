...
기존 ‘아직 안 된 것’ 목록은 이 시점의 아래 내용으로 보완한다.

- 홈·검색·공개 감찰 리포트를 새로 구성했다. 홈/리포트에 공통 장단점 카드를 사용한다.
- 지적/이력 저장 서비스와 migration 010이 저장소에 존재한다. 공개 분석 API에도 지적/이력을 연결했다. 운영 DB 적용 여부는 별도 확인이 필요하다.
- 소비자 리포트는 업체 전용 `/report` 대신 공개 `/trust-analysis`를 사용한다. 조회 실패와 실제 리뷰 0건을 구분한다.
- 미언급 지적을 자동으로 fixed 처리하던 동작은 중단했다. 항목별 재감찰 증거 API는 아직 필요하다. 과거 fixed는 재확인 필요한 기록으로 표현한다.
- 전체 화면군·결제/정산·후속 데이터 계약은 `../docs/PRODUCT_SPEC.md`를 참조한다.
- 생성 시안은 실제 운영 화면이 아니다. 카드 사진/예시 수치/확정 개선 배지를 API 없는 상태에서 가져오지 않는다.
